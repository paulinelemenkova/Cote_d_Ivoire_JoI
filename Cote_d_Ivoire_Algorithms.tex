...
terrain modelling comprising the geomorphic visualization of slope, aspect and elevation for relief analysis. 

%\newpage
\section{Study Area}

The study ares is focused on Côte d'Ivoire, West Africa, Figure \ref{fig1}. 
...
Plotting the colour composite of the Landsat OLI/TIRS image is based on the combinations of the three channel sensor bands used as elements of matrix scene. The three bands assigned to each of the three colour primaries (RGB) displayed the image, Listing \ref{lst:06}.
...
Computing vegetation indices is an important environmental tool based on the information from pixel value, which in turn is based on spectral reflectance of bands measured by the sensor. It is commonly computed with a spatial statistics and algebra of bands, using cell values defined on the scene, Listing \ref{lst:07}. Through a series of interactions among scene elements, they  are allocated to the classes based on the weighted values of their relative proportions.
