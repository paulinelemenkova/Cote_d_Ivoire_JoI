...
Accurate processing of satellite image is essential to the environmental modelling and mapping, as remote sensing data are the important source of spatial information derived from Earth observation. The main focus for image processing is the deriving of information from the raw data in raster format. In this context, the search for effective methods of image processing resulted in the variety of existing GIS software with specially designed embedded functions for raster data processing. However, in many practical situation, GIS present limitations either due to the commercial licence of software or functional restrictions. Accordingly, open source programming solutions are critical to process satellite imagery, since recent studies revealed that scripting techniques enable to handle Earth observation data effectively and accurately \cite{ELLEFSEN20191171,Lemenkova202212,Lemenkova202216,4160259}. 

Repeated structured representing similar objects on the Earth's surface are common in many types of satellite images as pixels with comparable ranges of values. These are especially prevailing in regions with distinct land cover types usually depicting similar or comparable land cover structure. Using this information, satellite images have been traditionally used for mapping  land cover fractions \cite{COULIBALY2021108092}, forests and vegetation types \cite{7729337}, hydrologic and geomorphic modelling \cite{2015hydrologie}, detecting changes in urban areas, mapping geological objects and mining sites\cite{NGOM2022102873}. Identifying and recognising feature objects is possible using the value of pixel brightness, intensity and textural features \cite{8519176}. Hence, algorithms of image classification and processing are based on detecting the pixels and classifying the image using binary data derived from raster cells. Algorithms of image classification and processing are based on reading the information from the pixels comprising the image. The analysis of the binary data derived from raster cells enables  classifying the image and mapping diverse phenomena, processes and objects based on image interpretation.

Python programming is one of the most important tools that can be used for modelling Earth phenomena using the libraries specially designed for spatial data analysis \cite{GUPTA2022100817,Lemenkova2020d,Ahmed}. The land cover properties which can be depicted  in the images and the described information can be mapped using the properties of pixels which mainly depend on the origin, mineral composition, texture, and land cover and fabric of the underlying Earth \cite{9066075,YUAN2022106966}. Besides the satellite images, the information can be derived from the LiDAR data whic hdepict the topographic shape of the Earth \cite{SHARMA2021317,8978301,9213897,YEH2017236,9553919,LI201955} 

Python is undoubtedly one of the most successful high-level programming languages \cite{van1995python}. Image and signal processing by object-oriented algorithms of Python is pervasive in Earth sciences and provides solutions to a wide class of problems in a variety of domains. For instance, Python has applications in geodetic studies \cite{Zhou2021GPS2spaceAO}, topographic analysis \cite{9254627}, hydrogeological modelling \cite{Mogaji}, modelling air pollution and land use \cite{Maetal2020}, batch spatial data processing \cite{6057428}, creating panorama images \cite{JASKOLKA2019e02560}, scanning tunneling microscopy \cite{BASTIDAS2021150821}, to mention a few. An essential advantage of Python consists in its compatibility: in contrast to stand along software, it can be integrated with other GIS via modules, plugins and toolkits \cite{6407912,7378577,VOS2019104528}. Novel libraries of Python enable to perform various tasks of image processing and spatial analysis \cite{Rey2010PySALAP,Rey2010}. Recent selected examples include image interpolation, calibration and correlation \cite{DEDEUSFILHO2022101131}, compressed sensing and image reconstruction \cite{FARRENS2020100402}, identifying and extracting vertical features \cite{GAO2022105503}, meteorological modelling \cite{6528254} and many more. The rich collection of its constantly developing libraries \cite{ROBERTS2022105192,CHEN2022105362} has recently built up with EarthPy, tailored for a wide variety of spatial tasks and geo-information data processing \cite{wasser2019EarthPyAP}. 

Yet another fundamental tool for spatial data processing is presented by R language \cite{RCoreTeam}, which also accounts for different statistical components available, a variety of its packages and rich graphical functionality \cite{Murrell}. 
...
Urban and industrial growth are exceptional for Côte d'Ivoire since 1950s, due to the demographic growth and population acceleration in the urban agglomerations \cite{Dubresson,Chaleard,Cotten1974,Cotten1973}. Geographically, this results in the spatially increased areas of cities and modified transport network \cite{Cotten1985}. Other anthropogenic activities include gold exploration and mining \cite{SAKO2018297,SAUERWEIN2020104323} and limited fishing restricted by narrow continental shelf \cite{Sournia}. Recently, Côte d'Ivoire experiences environmental changes common for humid West Africa, which include deforestation and fragmentation of ecosystems \cite{Chatelainetal2010}, fluctuations in species richness in savannah biomes \cite{Kolongo}, and increase in habitat heterogeneity which affects biodiversity and species community structure \cite{Yeo}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
