...
\AuthorCitation{\hl{Lemenkova, P.;} %MDPI: Please carefully check the accuracy of names.
 Debeir, O.}
% If this is a Chicago style journal: Lastname, Firstname, Firstname Lastname, and Firstname Lastname.

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address[1]{\highlighting{Université}%MDPI: please confirm address order according to order suggestion (no order requirement within the same pair of parentheses): (Laboratory | Group | Programme | Unit) < (Department | Division | Faculty | Institute | School) < (College | University | Universiti)
 Libre de Bruxelles, École Polytechnique de Bruxelles (EPB, Brussels Faculty of Engineering), \hl{Laboratory} of Image Synthesis and Analysis, Building L, \hl{Campus} de Solbosch, ULB--LISA CP165/57, Avenue~Franklin D. Roosevelt 50, B-1050 Brussels, Belgium; \hl{olivier.debeir@ulb.be} %MDPI: We added the email addresses here according to those submitted online at susy.mdpi.com. Please confirm.
}
...
\PACS{\hl{91.10.Da; 91.10.Jf; 91.10.Sp; 91.10.Xa; 96.25.Vt; 91.10.Fc; 95.40.+s; 95.75.Qr; 95.75.Rs; 42.68.Wt}%MDPI: please confirm if these three parts need keep
}
...
\begin{figure}[H]
     ...
     \begin{subfigure}[r]{0.60\textwidth}
         ...
         \caption{\centering}
     \end{subfigure}
        \caption{Data captured from the USGS Global Visualization Viewer (GloVis)'s satellite image repository, accessing remote sensing data from Landsat 9 OLI/TIRS C2 L1 and SRTM DEM for terrain analysis. ({\bf a})\hl{~Collected from Landsat 9 OLI/TIRS C2 L1.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
\begin{lstlisting}[language=bash, caption=\hl{Unix} %MDPI: please check and confirm all the listing format.
...
\begin{figure}[H]
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \caption{   \centering}
     \end{subfigure}
        \caption{\hl{Correlation } %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
between the selected Landsat-8 OLI/TIRS C1 bands in a multispectral image \emph{LC09\_L1TP\_197055\_20220111\_20220122\_02\_T1\_B}. ({\bf a}) \hl{Band 1 against Band 2.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
\caption{False color composite visualisation of Landsat 8 OLI image of Kossou Lake surroundings in Côte d'Ivoire. Mapping: RStudio. Source: authors. ({\bf a}) \hl{Bands 5, 7 and 3.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
\makebox[0.97\linewidth][r]{%
	...
	\begin{subfigure}[b]{.61\textwidth}
		...
		\caption{\centering}
	\end{subfigure}%
} %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
       \caption{\hl{NDVI} computed for multispectral {Landsat-9 OLI/TIRS} image.~({\bf a}) \hl{NDVI ratio-based vegetation index.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
The Soil-Adjusted Vegetation Index (SAVI) is a distance-based vegetation index which is based on the NDVI but includes an adjustment factor to reduce the noise from the NDVI by eliminating the effects from the canopy background and atmospheric conditions. Thus, it adjusts the NDVI for soil brightness where vegetation coverage is low in urban, densely built up areas and bare soil using the arithmetic expression in Equation (\ref{eq:2}), proposed \hl{in}%MDPI: please confirm if * in equation should be correct to \times
~\cite{HUETE1988295}:
...
\makebox[0.97\linewidth][r]{%
	...
	\end{subfigure}%
       ...
       \caption{\hl{SAVI } %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
 %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
computed for multispectral image \emph{LC09\_L1TP\_197055\_20220111\_20220122\_02\_T1\_B}. ({\bf a}) \hl{SAVI ratio-based vegetation index.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
\makebox[0.97\linewidth][r]{%
	...
	\end{subfigure}%
       ...
       \caption{\hl{EVI2, } %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
 %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
computed for multispectral image \emph{LC09\_L1TP\_197055\_20220111\_20220122\_02\_T1\_B}. ({\bf a}) \hl{EVI2 ratio-based vegetation index.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
...
\makebox[0.97\linewidth][r]{%
	...
	\end{subfigure}%
       ...
       \caption{\hl{Atmospherically} %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
%MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
 Resistant Vegetation Index 2 (ARVI2) computed for multispectral image \emph{LC09\_L1TP\_197055\_20220111\_20220122\_02\_T1\_B}. ({\bf a}) \hl{ARVI2 ratio-based vegetation index.} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
 ({\bf b}) Data distribution within the computed EVI2. }
        \label{fig11}
\end{figure}
...
%        		\includegraphics[width=8.6cm]{Fig_12a}
 ...
 %        \includegraphics[width=7cm]{Fig_12b}
     ...
     \end{subfigure}
        ...
        \caption{(\textbf{a}): \hl{Landsat OLI/TIRS} %MDPI: Please use commas to separate thousands for numbers with five or more digits (not four digits) in the picture, e.g., "10000" should be "10,000".
...
\caption{Terrain modelling for Kossou Lake area, Côte d'Ivoire. Mapping: Python. Source: authors. (\textbf{a}) \hl{DTM of Kossou Lake,} %MDPI: We moved the subfigure explanations into the figure caption. Please confirm.
 Côte d'Ivoire; 
...
\funding{This research received partial funding from the Multidisciplinary Digital Publishing Institute (MDPI).}
...
\newblock In Proceedings of the 2016 IEEE International Geoscience and Remote
  Sensing Symposium (IGARSS),  \hl{Beijing, China, 10--15 July }2016; pp. 1325--1328.
\newblock {https://doi.org/10.1109/IGARSS.2016.7729337}. %MDPI: newly added information, please confirm.
...
\newblock In Proceedings of the 2014 IEEE Geoscience and Remote Sensing
  Symposium,  \hl{Quebec City, QC, USA, 13--18 July} 2014; pp. 757--760.
\newblock {https://doi.org/10.1109/IGARSS.2014.6946534}.
...
\newblock In Proceedings of the 2012 IEEE International Geoscience and Remote
  Sensing Symposium,  \hl{Munich, Germany, 22--27 July} 2012; pp. 6279--6282.
\newblock {https://doi.org/10.1109/IGARSS.2012.6352694}.
...
\newblock {\em J. Afr. Earth Sci.} {\bf 2015}, {\em
  112},~440--450.
\newblock \hl{Tectonics, mineralisation and regolith evolution of the West African
  Craton}%MDPI: please confirm if this sentence could be deleted
...
\newblock \hl{SI: Remote Sensing and GIS Applications in Quaternary Sciences}%MDPI: please confirm if this sentence should be deleted
.
...
\newblock In Proceedings of the 2019 Global Conference for Advancement in
  Technology (GCAT),  \hl{Bangaluru, India, 18--20 October} 2019; pp. 1--4.
\newblock {https://doi.org/10.1109/GCAT47503.2019.8978301}.
...
\newblock In Proceedings of the 2020 International Conference on Unmanned
  Aircraft Systems (ICUAS),  \hl{Athens, Greece, 1--4 September} 2020; pp. 633--640.
\newblock {https://doi.org/10.1109/ICUAS48674.2020.9213897}.
...
\newblock In Proceedings of the 2021 IEEE International Geoscience and Remote
  Sensing Symposium IGARSS,  \hl{Brussels, Belgium, 11--16 July} 2021; pp. 8604--8607.
\newblock {https://doi.org/10.1109/IGARSS47720.2021.9553919}.
...
\newblock In Proceedings of the International Conference on Computational
  Intelligence and Multimedia Applications (ICCIMA 2007),  \hl{Sivakasi,  India, 13--15 December} 2007; Volume~3, pp.
  507--512.
\newblock {https://doi.org/10.1109/ICCIMA.2007.359}.
...
\newblock In Proceedings of the IGARSS '93---IEEE International
  Geoscience and Remote Sensing Symposium,  \hl{Tokyo, Japan, 18--21 August} 1993; Volume 4, pp. 1969--1971 .
\newblock {https://doi.org/10.1109/IGARSS.1993.322052}.
...
\newblock In Proceedings of the 2010 International Conference on Modern
  ...
  (TCSET),  \hl{Lviv, Ukraine, 23--27 February} 2010; pp. 56--58.
...
\newblock In Proceedings of the 2008 19th International Conference on Pattern
  Recognition,  \hl{Piscataway, NJ, USA, 8--11 December} 2008; pp. 1--4.
\newblock {https://doi.org/10.1109/ICPR.2008.4761020}.
...
\newblock In Proceedings of the IGARSS 2018---2018 IEEE International
  Geoscience and Remote Sensing Symposium, \hl{Valencia, Spain, 22--27 July} 2018; pp. 826--829.
\newblock {https://doi.org/10.1109/IGARSS.2018.8519176}.
...
\newblock In Proceedings of the IEEE SoutheastCon 2002 (Cat.
  No.02CH37283), \hl{Valencia, Spain, 5--7 April} 2002; pp. 62--66.
\newblock {https://doi.org/10.1109/SECON.2002.995559}.
...
\newblock In Proceedings of the 2000 International Conference on
  Image Processing (Cat. No.00CH37101),  10--13 September \hl{2000}%MDPI: Please add the location of the conference.
; Volume~2, pp. 732--735.
...
\newblock In Proceedings of the 2019 IEEE 4th International Conference on
  Signal and Image Processing (ICSIP),  \hl{Wuxi, China, 19--21 July} 2019; pp. 907--910.
\newblock {https://doi.org/10.1109/SIPROCESS.2019.8868688}.
...
\newblock \hl{15th European Symposium on Calcium}. %MDPI: Please check if this could be removed.
...
\newblock In Proceedings of the 2014 The Third International Conference on
  Agro-Geoinformatics, \hl{Beijing, China, 11--14 August} 2014; pp. 1--5.
\newblock {https://doi.org/10.1109/Agro-Geoinformatics.2014.6910592}.
...
\newblock In Proceedings of the 2012 Third International Conference on Emerging
  Applications of Information Technology, \hl{Kolkata, India, 16--18 April} 2012; pp. 235--238.
\newblock {https://doi.org/10.1109/EAIT.2012.6407912}.
...
\newblock In Proceedings of the 2015 23rd International Conference on
  Geoinformatics,  \hl{Wuhan, China, 19--21 June} 2015; pp. 1--6.
\newblock {https://doi.org/10.1109/GEOINFORMATICS.2015.7378577}.}
...
\newblock In Proceedings of the 2003 International Conference on
  Image Processing (Cat. No.03CH37429), \hl{Seville, Spain, 14--17 September} 2003; Volume~3, p. 1049.
\newblock {https://doi.org/10.1109/ICIP.2003.1247428}.
...
\newblock In Proceedings of the 2012 5th Romania Tier 2 Federation Grid, Cloud
  \& High Performance Computing Science (RQLCG),  \hl{Cluj-Napoc, Romania, 25 October} 2012; pp. 90--92;
\newblock {INSPEC Accession No: 13579885}.
...
\newblock In Proceedings of the 2019 42nd International Convention on
  ...
  (MIPRO),  \hl{Opatija, Croatia, 20--24 May} 2019; pp. 977--982.
...
\newblock In Proceedings of the 2019 Fifth International Conference on Advances
  in Biomedical Engineering (ICABME), \hl{Tripoli, Lebanon, 17--19 October} 2019; pp. 1--3.
\newblock {https://doi.org/10.1109/ICABME47164.2019.8940275}.
...
\newblock In Proceedings of the 2008 5th IEEE International Symposium on
  Biomedical Imaging: From Nano to Macro, \hl{Paris, France, 14--17 May} 2008; pp. 724--727.
\newblock {https://doi.org/10.1109/ISBI.2008.4541098}.
...
\newblock In Proceedings of the 2021 IEEE International Conference on
  Intelligent Systems, Smart and Green Technologies (ICISSGT), \hl{Visakhapatnam, India, 13--14 November} 2021; pp.
  167--170.
\newblock {https://doi.org/10.1109/ICISSGT52025.2021.00043}.
...
\newblock In Proceedings of the {2020 4th International Symposium on
  Multidisciplinary Studies and Innovative Technologies (ISMSIT)}, \hl{Ankara, Turkey, 22--24 October} 2020; pp.
  1--5.
\newblock {https://doi.org/10.1109/ISMSIT50672.2020.9254627}.
...
\newblock In Proceedings of the 2011 International Conference on Electrical and
  Control Engineering, \hl{Yichang, China, 16--18 September} 2011; pp. 4866--4869.
\newblock {https://doi.org/10.1109/ICECENG.2011.6057428}.
...
\newblock In Proceedings of the 2020 IEEE International Conference on Big Data
  (Big Data), \hl{Atlanta, GA, USA, 10--13 December} 2020; pp. 399--405.
\newblock {https://doi.org/10.1109/BigData50022.2020.9378399}.
...
\newblock In Proceedings of the 2022 International Conference on Electronics
  and Renewable Systems (ICEARS), \hl{Tuticorin, India, 16-18 March} 2022; pp. 1584--1587.
\newblock {https://doi.org/10.1109/ICEARS53579.2022.9752002}.
...
\newblock In Proceedings of the 2013 IEEE International Congress on Big Data,
  \hl{Santa Clara, CA, USA, 27 June--2July}  2013; pp. 423--424.
\newblock {https://doi.org/10.1109/BigData.Congress.2013.67}.
...
\newblock In Proceedings of the 2017 29th Chinese Control And Decision
  Conference (CCDC), \hl{Chongqing, China, 28--30 May} 2017; pp.~6806--6810.
\newblock {https://doi.org/10.1109/CCDC.2017.7978404}.
...
\newblock In Proceedings of the 2021 5th International Conference on
  Electronics, Communication and Aerospace Technology (ICECA), \hl{Coimbatore, India, 1--3 December} 2021; pp.
  1162--1165.
\newblock {https://doi.org/10.1109/ICECA52323.2021.9675988}.
...
\newblock In {\em SAR Image Analysis---A Computational Statistics Approach:
  With R Code, Data, and Applications}; Willey:  \hl{Hoboken, NJ, USA,} %newly added information, please confirm
 2022; pp. 43--60.
...
\newblock Available online: \url{https://doi.org/10.5285/a29c5465-b138-234d-e053-6c86abc040b9} (accessed on \hl{day month year).}%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\newblock \url{https://CRAN.R-project.org/package=raster} (accessed on \hl{day month year).}%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.

\bibitem[Neuwirth(2014)]{Neuwirth}
Neuwirth, E.
\newblock RColorBrewer: ColorBrewer Palettes. R Package Version 1.1-2. 2014.
\newblock Available online:  
\newblock \url{https://CRAN.R-project.org/package=RColorBrewer} (accessed on \hl{day month year).}%MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
...
\newblock \hl{Evolution of the East African and Related Orogens, and the Assembly
  of Gondwana,} {https://doi.org/10.1016/S0301-9268(03)00070-6}. %MDPI: Please check if this could be removed.
...
Gillies, S.
\newblock Rasterio: geospatial raster I/O for {Python} programmers,  \hl{2013}%MDPI: Please provide more information about the article type, such as book (please provide the name and location of the publisher); online resource (please provide the URL of the website and the date it was accessed (Date Month Year)); or journal article (please provide the name of the journal, the year and volume in which it was published, and the page number). Please refer to https://www.mdpi.com/authors/references for full reference formatting guides.
.
